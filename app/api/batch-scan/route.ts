...
export async function POST(req: NextRequest) {
  try {
    ...
    // Step 1: Crawl all sites in parallel (batches of 5 to not overwhelm the worker)
    ...
    for (let i = 0; i < validCrawls.length; i += 3) {
      ...
      const batchResults = await Promise.allSettled(
        batch.map(async (crawl) => {
          // Individual Gemini call — same as main page duelly-scan
          ...
          // Inject AI results — same logic as duelly-scan route
          ...
          return {
            url: crawl.url,
            seoScore: graded.seoScore,
            geoScore: graded.geoScore,

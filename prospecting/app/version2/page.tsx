...
import { Search, Loader2, MapPin, Building2, Globe, XCircle, TrendingUp } from "lucide-react"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"

interface ScanResult {
  url: string; seoScore: number; geoScore: number; domainAuthority: number; criticalIssues: string[]; error?: string
}

function ScoreGauge({ value, label, sublabel }: { value: number; label: string; sublabel?: string }) {
  const c = Math.max(0, Math.min(100, value));
  const hex = c >= 60 ? "#2ECC71" : c >= 30 ? "#F97316" : "#E05D5D";
  const r = 28, circ = 2 * Math.PI * r, off = circ - (c / 100) * circ;
  return (
    <div className="flex flex-col items-center gap-1">
      <div className="relative w-[68px] h-[68px]">
        <svg className="w-full h-full -rotate-90" viewBox="0 0 68 68">
          <circle cx="34" cy="34" r={r} fill="none" strokeWidth="5" className="stroke-muted/30" />
          <circle cx="34" cy="34" r={r} fill="none" strokeWidth="5" strokeLinecap="round" stroke={hex} strokeDasharray={circ} strokeDashoffset={off} style={{ transition: "stroke-dashoffset 0.6s ease" }} />
        </svg>
        <span className="absolute inset-0 flex items-center justify-center text-base font-bold" style={{ color: hex }}>{c}</span>
      </div>
...
const CATS = ["Restaurants & Cafes","Hair & Beauty Salons","Contractors & Trades","Plumbers","Electricians","HVAC","Roofing","Real Estate Agents","Auto Services","Healthcare & Medical","Dentists","Chiropractors","Retail Stores","Home Services","Landscaping","Fitness & Gyms","Pet Services","Photography","Accounting & Tax","Law Firms","Insurance Agents","Cleaning Services"]
...
export default function Version2Page() {
  ...
  const handleSearch = async () => {
    ...
    setSearching(true); setError(null); setScanResults(new Map()); setScanTime(null)
    try {
      const res = await fetch("/api/search", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ location: location.trim(), category, radius: 15 }) })
      if (!res.ok) throw new Error((await res.json()).error || "Search failed")
      setBusinesses((await res.json()).businesses || [])
    } catch (err: any) { setError(err.message) }
    setSearching(false)
  }

  const handleBatchScan = async () => {
    const sites = businesses.filter(b => b.webPresence === "website" && b.website)
    if (sites.length === 0) return
    setScanning(true); setError(null)
    const start = Date.now()
    try {
      const res = await fetch("/api/batch-scan", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ urls: sites.map(b => b.website) }) })
      if (!res.ok) throw new Error((await res.json()).error || "Batch scan failed")
      const data = await res.json()
      setScanTime(parseFloat(((Date.now() - start) / 1000).toFixed(1)))
  ...
  const withSites = businesses.filter(b => b.webPresence === "website" && b.website)
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <Input placeholder="City, State (e.g. Dallas, TX)" value={location} onChange={(e) => setLocation(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSearch()} className="pl-9" />
            </div>
            <Select value={category} onValueChange={setCategory}>
              <SelectTrigger className="sm:w-64"><Building2 className="w-4 h-4 mr-2 text-muted-foreground" /><SelectValue placeholder="Select a category" /></SelectTrigger>
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {withSites.map(b => {
                  ...
                  const scan = scanResults.get(url)
                  return (
